...
uses
  ...
  unitConfiguracoes, ACBrBase, ACBrValidador;
...
uses untPesquisarClientes, untTelaDePedido;
...
begin
   ...
   End else if tipo = 'D' then Begin
      ...
      mkeDataNacimento.Enabled:= false;
...
begin
   ...
   if frmPrincipal.qryParametrosVALIDAR_CPF_CLIENTE.asstring = 'S' then Begin
      if rdbCPF.Checked then
         begin
            ACBrValidador1.TipoDocto := docCPF;
            ACBrValidador1.Documento := Trim(edtCPF_CNPJ.Text);
   ...
   Try
      ...
      Try
         cliente.DataNascimento := StrToDate(mkeDataNacimento.Text);
      Except
      End;
