...
uses untTelaDePedido, untPedidos, untFinalizarVenda, untControleFiado;

procedure TfrmObservacao.Confirmar;
begin

   if tipo = 'INFADCIONAIS' then
      frmFinalizarVenda.infAdicional := Trim(mmoObs.Lines.Text)
   else
   if tipo = 'F' then Begin
      frmFinalizarVenda.obsFiado := Trim(mmoObs.Lines.Text);
   End else if tipo = 'V' then Begin
      frmPedidos.qryItens.Edit;
      frmPedidos.qryItensobservacao.AsString := Trim(mmoObs.Lines.Text);
      frmPedidos.qryItens.Post;
   end else if not Assigned(frmControleFiado) then
