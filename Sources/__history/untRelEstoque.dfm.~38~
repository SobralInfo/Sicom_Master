object frmRelEstoque: TfrmRelEstoque
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmRelEstoque'
  ClientHeight = 409
  ClientWidth = 377
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Image2: TImage
    Left = 35
    Top = 343
    Width = 289
    Height = 39
    Picture.Data = {
      
      AE426082}
  end
  object Image3: TImage
    Left = 41
    Top = 350
    Width = 148
    Height = 31
    OnClick = Image3Click
  end
  object Image4: TImage
    Left = 193
    Top = 350
    Width = 131
    Height = 31
    Cursor = crHandPoint
  end
  object Label10: TLabel
    Left = 20
    Top = 48
    Width = 55
    Height = 16
    Caption = 'Categoria'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label1: TLabel
    Left = 20
    Top = 112
    Width = 44
    Height = 16
    Caption = 'Produto'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object SpeedButton5: TSpeedButton
    Left = 333
    Top = 134
    Width = 23
    Height = 24
    Flat = True
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
    NumGlyphs = 2
    OnClick = SpeedButton5Click
  end
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 377
    Height = 31
    Align = alTop
    BevelOuter = bvNone
    Color = 6710886
    ParentBackground = False
    TabOrder = 0
    ExplicitWidth = 378
    object Label2: TLabel
      Left = 20
      Top = 8
      Width = 45
      Height = 16
      Caption = 'Estoque'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Image1: TImage
      Left = 350
      Top = 4
      Width = 22
      Height = 22
      Cursor = crHandPoint
      Picture.Data = {
        
        0000000049454E44AE426082}
      Stretch = True
      OnClick = Image1Click
    end
  end
  object Panel5: TPanel
    Left = 0
    Top = 31
    Width = 1
    Height = 378
    Align = alLeft
    BevelOuter = bvNone
    Color = 6710886
    ParentBackground = False
    TabOrder = 1
  end
  object chkAtivos: TCheckBox
    Left = 20
    Top = 189
    Width = 145
    Height = 17
    Caption = 'Somente Produtos Ativos'
    TabOrder = 3
    OnClick = chkAtivosClick
  end
  object chkInativos: TCheckBox
    Left = 20
    Top = 212
    Width = 145
    Height = 17
    Caption = 'Somente Produtos Inativos'
    TabOrder = 4
    OnClick = chkInativosClick
  end
  object chkComSaldo: TCheckBox
    Left = 20
    Top = 236
    Width = 193
    Height = 17
    Caption = 'Somente Produtos com Saldo '
    TabOrder = 5
    OnClick = chkComSaldoClick
  end
  object chkEstoqueMinimo: TCheckBox
    Left = 20
    Top = 282
    Width = 258
    Height = 17
    Caption = 'Somente Produtos c/ Estoque Abaixo do M'#237'nimo'
    TabOrder = 6
  end
  object chkSemSaldo: TCheckBox
    Left = 20
    Top = 259
    Width = 169
    Height = 17
    Caption = 'Somente Produtos sem Saldo '
    TabOrder = 7
    OnClick = chkSemSaldoClick
  end
  object lkpCategoria: TDBLookupComboBox
    Left = 20
    Top = 70
    Width = 336
    Height = 24
    Color = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'ID'
    ListField = 'DESCRICAO'
    ListSource = dtsCategoria
    ParentFont = False
    TabOrder = 8
  end
  object chkPromocao: TCheckBox
    Left = 20
    Top = 305
    Width = 193
    Height = 17
    Caption = 'Somente Produtos em Promo'#231#227'o'
    TabOrder = 9
  end
  object Panel1: TPanel
    Left = -512
    Top = 388
    Width = 836
    Height = 287
    TabOrder = 2
    Visible = False
    object RLReport1: TRLReport
      Left = 13
      Top = 12
      Width = 794
      Height = 1123
      DataSource = dtsRel
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      object RLBand1: TRLBand
        Left = 38
        Top = 38
        Width = 718
        Height = 111
        BandType = btHeader
        Borders.Sides = sdCustom
        Borders.DrawLeft = True
        Borders.DrawTop = True
        Borders.DrawRight = True
        Borders.DrawBottom = True
        object lblFiltro01: TRLLabel
          Left = 8
          Top = 62
          Width = 200
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object lblFiltro02: TRLLabel
          Left = 8
          Top = 84
          Width = 200
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object lblFiltro03: TRLLabel
          Left = 508
          Top = 84
          Width = 200
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object lblFiltro04: TRLLabel
          Left = 508
          Top = 62
          Width = 200
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object lblFiltro05: TRLLabel
          Left = 221
          Top = 84
          Width = 270
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object lblFiltro06: TRLLabel
          Left = 221
          Top = 62
          Width = 270
          Height = 16
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
        end
        object RLLabel1: TRLLabel
          Left = -1
          Top = 9
          Width = 720
          Height = 18
          Alignment = taCenter
          AutoSize = False
          Caption = 'Relat'#243'rio de Estoque'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object RLSystemInfo2: TRLSystemInfo
          Left = 231
          Top = 28
          Width = 257
          Height = 16
          Alignment = taCenter
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Info = itNow
          ParentFont = False
          Text = 'Data/Hora Impress'#227'o: '
        end
      end
      object RLBand2: TRLBand
        Left = 38
        Top = 167
        Width = 718
        Height = 22
        object RLDBText1: TRLDBText
          Left = 8
          Top = 3
          Width = 299
          Height = 16
          AutoSize = False
          DataField = 'DESCRICAO'
          DataSource = dtsRel
          Text = ''
        end
        object RLDBText2: TRLDBText
          Left = 315
          Top = 3
          Width = 82
          Height = 16
          Alignment = taRightJustify
          AutoSize = False
          DataField = 'QTDE_ESTOQUE'
          DataSource = dtsRel
          Text = ''
        end
        object RLDBText3: TRLDBText
          Left = 537
          Top = 3
          Width = 82
          Height = 16
          Alignment = taRightJustify
          AutoSize = False
          DataField = 'calc_valor_medio'
          DataSource = dtsRel
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 3684408
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          Text = ''
        end
        object RLDBText4: TRLDBText
          Left = 446
          Top = 3
          Width = 82
          Height = 16
          Alignment = taRightJustify
          AutoSize = False
          DataField = 'VALOR_UNITARIO'
          DataSource = dtsRel
          Text = ''
        end
        object RLDBText5: TRLDBText
          Left = 625
          Top = 3
          Width = 84
          Height = 16
          Alignment = taCenter
          AutoSize = False
          DataField = 'calc_data_ult_entrada'
          DataSource = dtsRel
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 3684408
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          Text = ''
        end
        object RLDBText6: TRLDBText
          Left = 404
          Top = 3
          Width = 36
          Height = 16
          AutoSize = False
          DataField = 'SIGLA'
          DataSource = dtsRel
          Text = ''
        end
      end
      object RLBand3: TRLBand
        Left = 38
        Top = 149
        Width = 718
        Height = 18
        BandType = btHeader
        Color = 15263976
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 14869218
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = False
        object RLLabel7: TRLLabel
          Left = 8
          Top = 2
          Width = 57
          Height = 14
          Caption = 'Descri'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
        object RLLabel5: TRLLabel
          Left = 318
          Top = 2
          Width = 79
          Height = 14
          Alignment = taRightJustify
          Caption = 'Estoque Atual'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
        object RLLabel6: TRLLabel
          Left = 404
          Top = 2
          Width = 47
          Height = 14
          Caption = 'Unidade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
        object RLLabel4: TRLLabel
          Left = 458
          Top = 2
          Width = 70
          Height = 14
          Alignment = taRightJustify
          Caption = 'Valor Venda'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
        object RLLabel3: TRLLabel
          Left = 550
          Top = 2
          Width = 69
          Height = 14
          Alignment = taRightJustify
          Caption = 'Valor M'#233'dio'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
        object RLLabel2: TRLLabel
          Left = 635
          Top = 2
          Width = 64
          Height = 14
          Alignment = taCenter
          Caption = #218'lt. Entrada'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = False
        end
      end
      object RLBand5: TRLBand
        Left = 38
        Top = 214
        Width = 718
        Height = 29
        BandType = btFooter
        object RLSystemInfo1: TRLSystemInfo
          Left = 293
          Top = 3
          Width = 131
          Height = 16
          AutoSize = False
          Info = itPageNumber
          Text = 'P'#225'gina '
        end
      end
      object RLBand4: TRLBand
        Left = 38
        Top = 189
        Width = 718
        Height = 25
        BandType = btSummary
        Borders.Sides = sdCustom
        Borders.DrawLeft = False
        Borders.DrawTop = True
        Borders.DrawRight = False
        Borders.DrawBottom = False
        object RLDBResult1: TRLDBResult
          Left = 8
          Top = 6
          Width = 345
          Height = 15
          DataField = 'DESCRICAO'
          DataSource = dtsRel
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Info = riCount
          ParentFont = False
          Text = 'Total de Produtos: '
        end
        object RLDBResult2: TRLDBResult
          Left = 478
          Top = 6
          Width = 141
          Height = 15
          Alignment = taRightJustify
          DataField = 'calc_valor_medio'
          DataSource = dtsRel
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Info = riSum
          ParentFont = False
          Text = ''
        end
        object RLDBResult3: TRLDBResult
          Left = 387
          Top = 6
          Width = 141
          Height = 15
          Alignment = taRightJustify
          DataField = 'VALOR_UNITARIO'
          DataSource = dtsRel
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Info = riSum
          ParentFont = False
          Text = ''
        end
      end
    end
  end
  object edtProduto: TJvValidateEdit
    Left = 20
    Top = 134
    Width = 311
    Height = 24
    Alignment = taLeftJustify
    CheckChars = '01234567890'
    Color = clWhite
    CriticalPoints.MaxValueIncluded = False
    CriticalPoints.MinValueIncluded = False
    DisplayFormat = dfNone
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 10
    OnKeyDown = edtProdutoKeyDown
  end
  object dtsRel: TDataSource
    DataSet = qryRel
    Left = 408
    Top = 37
  end
  object qryAUX: TFDQuery
    Connection = frmPrincipal.ConexaoLocal
    FetchOptions.AssignedValues = [evRowsetSize]
    FetchOptions.RowsetSize = 1500000
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateMode = upWhereAll
    SQL.Strings = (
      
        '      SELECT Coalesce(cl.nome, '#39'Cliente n'#227'o informado'#39') AS clien' +
        'te,'
      '             c.data_hora_venda,'
      '             c.valor_venda,'
      '             c.desconto,'
      '             c.acrescimo,'
      '             c.valor_final'
      '        FROM venda_cabecalho c'
      '             LEFT JOIN clientes cl ON c.id_cliente = cl.id'
      '       WHERE c.status = '#39'F'#39
      '    ORDER BY c.data_hora_venda')
    Left = 496
    Top = 37
  end
  object qryRel: TFDQuery
    OnCalcFields = qryRelCalcFields
    Connection = frmPrincipal.ConexaoLocal
    FetchOptions.AssignedValues = [evRowsetSize]
    FetchOptions.RowsetSize = 1500000
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateMode = upWhereAll
    SQL.Strings = (
      '      SELECT p.*,'
      '             u.sigla,'
      '             c.descricao AS categoria'
      '        FROM produtos p'
      '             LEFT JOIN categoria c ON p.id_categoria = c.id,'
      '             unidade_produtos u'
      '       WHERE p.id_unidade = u.id')
    Left = 448
    Top = 37
    object qryRelID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryRelSTATUS: TStringField
      FieldName = 'STATUS'
      Origin = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object qryRelDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'DESCRICAO'
      Size = 120
    end
    object qryRelQTDE_ESTOQUE: TFloatField
      FieldName = 'QTDE_ESTOQUE'
      Origin = 'QTDE_ESTOQUE'
      DisplayFormat = '#0.000'
    end
    object qryRelREFERENCIA: TStringField
      FieldName = 'REFERENCIA'
      Origin = 'REFERENCIA'
      Size = 80
    end
    object qryRelID_UNIDADE: TIntegerField
      FieldName = 'ID_UNIDADE'
      Origin = 'ID_UNIDADE'
    end
    object qryRelCODIGO_BARRAS: TStringField
      FieldName = 'CODIGO_BARRAS'
      Origin = 'CODIGO_BARRAS'
      FixedChar = True
      Size = 15
    end
    object qryRelVALOR_UNITARIO: TFloatField
      FieldName = 'VALOR_UNITARIO'
      Origin = 'VALOR_UNITARIO'
      DisplayFormat = '###,#0.00'
    end
    object qryRelID_ORIGINAL: TStringField
      FieldName = 'ID_ORIGINAL'
      Origin = 'ID_ORIGINAL'
      Size = 80
    end
    object qryRelIMPRIMIR_COZINHA: TStringField
      FieldName = 'IMPRIMIR_COZINHA'
      Origin = 'IMPRIMIR_COZINHA'
      FixedChar = True
      Size = 1
    end
    object qryRelID_CATEGORIA: TIntegerField
      FieldName = 'ID_CATEGORIA'
      Origin = 'ID_CATEGORIA'
    end
    object qryRelSIGLA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'SIGLA'
      Origin = 'SIGLA'
      ProviderFlags = []
      ReadOnly = True
      Size = 15
    end
    object qryRelCATEGORIA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CATEGORIA'
      Origin = 'DESCRICAO'
      ProviderFlags = []
      ReadOnly = True
      Size = 80
    end
    object qryRelcalc_valor_medio: TFloatField
      FieldKind = fkCalculated
      FieldName = 'calc_valor_medio'
      DisplayFormat = '###,#0.00'
      Calculated = True
    end
    object qryRelcalc_data_ult_entrada: TStringField
      FieldKind = fkCalculated
      FieldName = 'calc_data_ult_entrada'
      Size = 15
      Calculated = True
    end
  end
  object qryCategoria: TFDQuery
    Connection = frmPrincipal.ConexaoLocal
    FetchOptions.AssignedValues = [evRowsetSize]
    FetchOptions.RowsetSize = 1500000
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateMode = upWhereAll
    SQL.Strings = (
      'SELECT id, descricao FROM categoria '
      'UNION'
      'SELECT 0 AS id, '#39'TODOS'#39' AS descricao FROM configuracoes'
      'ORDER BY 2')
    Left = 456
    Top = 117
    object qryCategoriaID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryCategoriaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'DESCRICAO'
      Size = 80
    end
  end
  object dtsCategoria: TDataSource
    DataSet = qryCategoria
    Left = 520
    Top = 125
  end
  object RLPDFFilter1: TRLPDFFilter
    DocumentInfo.Creator = 
      'FortesReport Community Edition v4.0 \251 Copyright '#169' 1999-2016 F' +
      'ortes Inform'#225'tica'
    DisplayName = 'Documento PDF'
    Left = 776
    Top = 56
  end
end
