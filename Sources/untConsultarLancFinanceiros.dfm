object frmConsultarLancFinanceiros: TfrmConsultarLancFinanceiros
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmConsultarLancFinanceiros'
  ClientHeight = 650
  ClientWidth = 1000
  Color = 6710886
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 1000
    Height = 31
    Align = alTop
    BevelOuter = bvNone
    Color = 6710886
    ParentBackground = False
    TabOrder = 0
    object Label2: TLabel
      Left = 11
      Top = 8
      Width = 202
      Height = 16
      Caption = 'Consultar Lan'#231'amentos Financeiros'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Image1: TImage
      AlignWithMargins = True
      Left = 973
      Top = 3
      Width = 22
      Height = 25
      Cursor = crHandPoint
      Margins.Right = 5
      Align = alRight
      Picture.Data = {
        
        0000000049454E44AE426082}
      Stretch = True
      OnClick = Image1Click
      ExplicitLeft = 951
    end
  end
  object Panel6: TPanel
    Left = 0
    Top = 647
    Width = 1000
    Height = 3
    Align = alBottom
    BevelOuter = bvNone
    Color = 6710886
    ParentBackground = False
    TabOrder = 1
  end
  object Panel2: TPanel
    Left = 0
    Top = 31
    Width = 1000
    Height = 616
    Align = alClient
    BevelOuter = bvNone
    Caption = 'Panel2'
    Color = clWhite
    ParentBackground = False
    TabOrder = 2
    object GroupBox1: TGroupBox
      AlignWithMargins = True
      Left = 3
      Top = 416
      Width = 991
      Height = 194
      Margins.Right = 6
      Margins.Bottom = 6
      Align = alBottom
      Caption = 'Detalhamento'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNone
      Font.Height = -9
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object Label1: TLabel
        Left = 8
        Top = 22
        Width = 92
        Height = 13
        Alignment = taRightJustify
        Caption = 'Filtros Aplicados'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 28
        Top = 46
        Width = 96
        Height = 13
        Caption = 'Cliente/Fornecedor:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label5: TLabel
        Left = 28
        Top = 69
        Width = 62
        Height = 13
        Caption = 'Vencimento: '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object lblFiltroClienteFornecedor: TLabel
        Left = 128
        Top = 46
        Width = 157
        Height = 13
        AutoSize = False
        Caption = 'TODOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblFiltrosPeriodo: TLabel
        Left = 91
        Top = 69
        Width = 168
        Height = 13
        AutoSize = False
        Caption = '22/09/2023 '#224' 22/09/2023'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label13: TLabel
        Left = 339
        Top = 22
        Width = 141
        Height = 13
        Alignment = taRightJustify
        Caption = 'Dados T'#237'tulo Selecionado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label14: TLabel
        Left = 356
        Top = 113
        Width = 60
        Height = 13
        Caption = 'Status T'#237'tulo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label15: TLabel
        Left = 419
        Top = 114
        Width = 69
        Height = 13
        AutoSize = False
        Caption = '.......................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label6: TLabel
        Left = 669
        Top = 22
        Width = 77
        Height = 13
        Alignment = taRightJustify
        Caption = 'Saldo Per'#237'odo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label7: TLabel
        Left = 679
        Top = 46
        Width = 83
        Height = 13
        Caption = 'Total de Receitas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label8: TLabel
        Left = 768
        Top = 49
        Width = 58
        Height = 13
        AutoSize = False
        Caption = '..................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object lblTotalReceitas: TLabel
        Left = 884
        Top = 46
        Width = 24
        Height = 13
        Alignment = taRightJustify
        Caption = '0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lblTotalDespesas: TLabel
        Left = 884
        Top = 68
        Width = 24
        Height = 13
        Alignment = taRightJustify
        Caption = '0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label12: TLabel
        Left = 775
        Top = 69
        Width = 51
        Height = 13
        AutoSize = False
        Caption = '.............'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label16: TLabel
        Left = 679
        Top = 68
        Width = 90
        Height = 13
        AutoSize = False
        Caption = 'Total de Despesas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label9: TLabel
        Left = 679
        Top = 90
        Width = 117
        Height = 13
        AutoSize = False
        Caption = 'Resultado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label10: TLabel
        Left = 731
        Top = 91
        Width = 95
        Height = 13
        AutoSize = False
        Caption = '...........................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object lblSaldo: TLabel
        Left = 884
        Top = 90
        Width = 24
        Height = 13
        Alignment = taRightJustify
        Caption = '0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label11: TLabel
        Left = 356
        Top = 135
        Width = 83
        Height = 13
        AutoSize = False
        Caption = 'Data Pagamento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label17: TLabel
        Left = 440
        Top = 136
        Width = 48
        Height = 13
        AutoSize = False
        Caption = '...................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label22: TLabel
        Left = 356
        Top = 157
        Width = 53
        Height = 13
        AutoSize = False
        Caption = 'Valor Pago'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label24: TLabel
        Left = 410
        Top = 157
        Width = 78
        Height = 13
        AutoSize = False
        Caption = '..................................................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object lblStatus: TLabel
        Left = 66
        Top = 94
        Width = 219
        Height = 13
        AutoSize = False
        Caption = 'TODOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label18: TLabel
        Left = 28
        Top = 94
        Width = 35
        Height = 13
        Caption = 'Status:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText1: TDBText
        Left = 495
        Top = 113
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'calcStatus'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText2: TDBText
        Left = 495
        Top = 135
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'DATA_PAGAMENTO'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText3: TDBText
        Left = 495
        Top = 157
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'VALOR_PAGO'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label19: TLabel
        Left = 356
        Top = 46
        Width = 64
        Height = 13
        Caption = 'Tipo do T'#237'tulo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label20: TLabel
        Left = 419
        Top = 47
        Width = 69
        Height = 13
        AutoSize = False
        Caption = '.......................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText4: TDBText
        Left = 495
        Top = 46
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'TIPOLANCAMENTO'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 356
        Top = 91
        Width = 35
        Height = 13
        Caption = 'Parcela'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label21: TLabel
        Left = 396
        Top = 92
        Width = 92
        Height = 13
        AutoSize = False
        Caption = '.......................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText5: TDBText
        Left = 495
        Top = 91
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'PARCELA'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label23: TLabel
        Left = 356
        Top = 68
        Width = 65
        Height = 13
        Caption = 'Forma Pagto.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label25: TLabel
        Left = 419
        Top = 69
        Width = 69
        Height = 13
        AutoSize = False
        Caption = '.......................'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText6: TDBText
        Left = 495
        Top = 68
        Width = 125
        Height = 17
        Alignment = taRightJustify
        DataField = 'FORMAPAGTO'
        DataSource = dtsListagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 4605510
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Panel4: TPanel
        Left = 316
        Top = 29
        Width = 1
        Height = 127
        TabOrder = 0
      end
      object Panel7: TPanel
        Left = 648
        Top = 29
        Width = 1
        Height = 127
        TabOrder = 1
      end
    end
    object GridList: TRxDBGrid
      AlignWithMargins = True
      Left = 6
      Top = 3
      Width = 988
      Height = 410
      Margins.Left = 6
      Margins.Right = 6
      Margins.Bottom = 0
      Align = alClient
      BorderStyle = bsNone
      DataSource = dtsListagem
      DefaultDrawing = False
      GradientStartColor = clActiveBorder
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit]
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnDrawColumnCell = GridListDrawColumnCell
      AutoAppend = False
      RowColorsUse = True
      RowColor1 = clWhite
      RowColor2 = clWhite
      Columns = <
        item
          Expanded = False
          FieldName = 'DESCRICAO_CLIENTE_FORNECEDOR'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Caption = 'Cliente/Fornecedor'
          Width = 277
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CONTAFINANCEIRA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Caption = 'Conta Financeira'
          Width = 160
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO_DOCUMENTO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Caption = 'Documento'
          Width = 113
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'calcStatus'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Caption = 'Status'
          Width = 102
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'DATA_ENTRADA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Data Entrada'
          Width = 106
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'DATA_VENCIMENTO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Data Vencimento'
          Width = 102
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_DOCUMENTO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Alignment = taRightJustify
          Title.Caption = 'Valor Documento'
          Width = 90
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'DATA_PAGAMENTO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Data Pagamento'
          Width = 107
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_PAGO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Calibri'
          Font.Style = []
          Title.Caption = 'Valor Pago'
          Visible = True
        end>
    end
  end
  object qryListagem: TFDQuery
    OnCalcFields = qryListagemCalcFields
    Connection = frmPrincipal.ConexaoLocal
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateMode = upWhereChanged
    SQL.Strings = (
      'Select lf.*,'
      '       cf.descricao as contaFinanceira, fp.nome as formaPagto,'
      
        '       case when lf.tipo_lancamento = '#39'R'#39' then '#39'RECEITA'#39' else '#39'D' +
        'ESPESA'#39' end as tipoLancamento'
      
        '  from lancamentos_financeiros lf inner join contas_financeiras ' +
        'cf on lf.id_conta_financeira = cf.id'
      
        '                                                      left join ' +
        'FORMAS_PAGAMENTO fp on lf.id_forma_pagto = fp.id'
      'where  lf.tipo_lancamento = '#39'Y'#39' '
      ' ')
    Left = 480
    Top = 120
    object qryListagemID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      Required = True
    end
    object qryListagemID_CONTA_FINANCEIRA: TIntegerField
      FieldName = 'ID_CONTA_FINANCEIRA'
      Origin = 'ID_CONTA_FINANCEIRA'
    end
    object qryListagemID_CLIENTE_FORNECEDOR: TIntegerField
      FieldName = 'ID_CLIENTE_FORNECEDOR'
      Origin = 'ID_CLIENTE_FORNECEDOR'
    end
    object qryListagemDESCRICAO_CLIENTE_FORNECEDOR: TStringField
      FieldName = 'DESCRICAO_CLIENTE_FORNECEDOR'
      Origin = 'DESCRICAO_CLIENTE_FORNECEDOR'
      Size = 120
    end
    object qryListagemTIPO_CLIENTE_FORNECEDOR: TStringField
      FieldName = 'TIPO_CLIENTE_FORNECEDOR'
      Origin = 'TIPO_CLIENTE_FORNECEDOR'
      FixedChar = True
      Size = 1
    end
    object qryListagemDESCRICAO_DOCUMENTO: TStringField
      FieldName = 'DESCRICAO_DOCUMENTO'
      Origin = 'DESCRICAO_DOCUMENTO'
      Size = 100
    end
    object qryListagemDATA_VENCIMENTO: TSQLTimeStampField
      FieldName = 'DATA_VENCIMENTO'
      Origin = 'DATA_VENCIMENTO'
    end
    object qryListagemDATA_ENTRADA: TSQLTimeStampField
      FieldName = 'DATA_ENTRADA'
      Origin = 'DATA_ENTRADA'
    end
    object qryListagemVALOR_DOCUMENTO: TFloatField
      FieldName = 'VALOR_DOCUMENTO'
      Origin = 'VALOR_DOCUMENTO'
      DisplayFormat = '###,#0.00'
    end
    object qryListagemOBSERVACOES: TStringField
      FieldName = 'OBSERVACOES'
      Origin = 'OBSERVACOES'
      Size = 3500
    end
    object qryListagemSTATUS: TStringField
      FieldName = 'STATUS'
      Origin = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object qryListagemDATA_PAGAMENTO: TSQLTimeStampField
      FieldName = 'DATA_PAGAMENTO'
      Origin = 'DATA_PAGAMENTO'
    end
    object qryListagemVALOR_PAGO: TFloatField
      FieldName = 'VALOR_PAGO'
      Origin = 'VALOR_PAGO'
      DisplayFormat = '###,#0.00'
    end
    object qryListagemID_TOTAL_TIPO_PAGTO: TIntegerField
      FieldName = 'ID_TOTAL_TIPO_PAGTO'
      Origin = 'ID_TOTAL_TIPO_PAGTO'
    end
    object qryListagemTIPO_LANCAMENTO: TStringField
      FieldName = 'TIPO_LANCAMENTO'
      Origin = 'TIPO_LANCAMENTO'
      FixedChar = True
      Size = 1
    end
    object qryListagemCONTAFINANCEIRA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CONTAFINANCEIRA'
      Origin = 'DESCRICAO'
      ProviderFlags = []
      ReadOnly = True
      Size = 80
    end
    object qryListagemTIPOLANCAMENTO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'TIPOLANCAMENTO'
      Origin = 'TIPOLANCAMENTO'
      ProviderFlags = []
      ReadOnly = True
      FixedChar = True
      Size = 7
    end
    object qryListagemcalcStatus: TStringField
      FieldKind = fkCalculated
      FieldName = 'calcStatus'
      Size = 18
      Calculated = True
    end
    object qryListagemPARCELA: TStringField
      FieldName = 'PARCELA'
      Origin = 'PARCELA'
      Size = 10
    end
    object qryListagemID_FORMA_PAGTO: TIntegerField
      FieldName = 'ID_FORMA_PAGTO'
      Origin = 'ID_FORMA_PAGTO'
    end
    object qryListagemFORMAPAGTO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'FORMAPAGTO'
      Origin = 'NOME'
      ProviderFlags = []
      ReadOnly = True
      Size = 120
    end
  end
  object dtsListagem: TDataSource
    AutoEdit = False
    DataSet = qryListagem
    Left = 552
    Top = 120
  end
  object PopupMenu1: TPopupMenu
    Left = 256
    Top = 112
    object NovoLa1: TMenuItem
      Caption = 'Novo T'#237'tulo'
      OnClick = NovoLa1Click
    end
    object EditarLanamento1: TMenuItem
      Caption = 'Alterar T'#237'tulo'
      OnClick = EditarLanamento1Click
    end
    object CancelarLanamento1: TMenuItem
      Caption = 'Cancelar T'#237'tulo'
      OnClick = CancelarLanamento1Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object LiquidarTtuloSelecionado1: TMenuItem
      Caption = 'Liquidar T'#237'tulo Selecionado'
      OnClick = LiquidarTtuloSelecionado1Click
    end
    object LiquidarTtulosConsultados1: TMenuItem
      Caption = 'Liquidar T'#237'tulos Consultados'
      OnClick = LiquidarTtulosConsultados1Click
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object FiltrarConsulta1: TMenuItem
      Caption = 'Filtrar Consulta'
      OnClick = FiltrarConsulta1Click
    end
  end
  object qryAUX: TFDQuery
    Connection = frmPrincipal.ConexaoLocal
    UpdateOptions.AssignedValues = [uvUpdateMode]
    UpdateOptions.UpdateMode = upWhereAll
    SQL.Strings = (
      'Select lf.*,'
      '       cf.descricao as contaFinanceira,'
      
        '       case when lf.tipo_lancamento = '#39'R'#39' then '#39'RECEITA'#39' else '#39'D' +
        'ESPESA'#39' end as tipoLancamento'
      
        '  from lancamentos_financeiros lf inner join contas_financeiras ' +
        'cf on lf.id_conta_financeira = cf.id')
    Left = 480
    Top = 176
  end
end
